(function(){
  ...
  function ensureOwnerNav(){
    ...
    const style=document.createElement('style');style.id='yocewor-owner-nav-lock';style.textContent=`
      .nav{position:sticky!important;top:0!important;z-index:9999!important;display:flex!important;flex-wrap:nowrap!important;align-items:center!important;gap:6px!important;white-space:nowrap!important;overflow-x:auto!important;overflow-y:hidden!important;width:100%!important;box-sizing:border-box!important;-webkit-overflow-scrolling:touch!important;scrollbar-width:none!important}
      .nav::-webkit-scrollbar{display:none!important}.nav>a,.nav>button{flex:0 0 auto!important;white-space:nowrap!important;display:inline-flex!important;align-items:center!important}
    ...
    if(location.pathname==='/gm-manage-users.html'){
      ...
      hideLogout();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',hideLogout);else setTimeout(hideLogout,0);return;
    }
    ...
    document.querySelectorAll('.nav').forEach(nav=>{if(nav.dataset.yoceworLocked==='1')return;nav.dataset.yoceworLocked='1';nav.innerHTML=html;const current=location.pathname.replace(/\/$/,'')||'/owner-dashboard.html';nav.querySelectorAll('a').forEach(a=>{if(a.getAttribute('href')===current)a.classList.add('active')});const lo=nav.querySelector('#yocewor-nav-logout');if(lo)lo.onclick=async e=>{e.preventDefault();await signOut();location.href='/'}})
  }

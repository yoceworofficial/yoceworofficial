/* YOCEWOR Team Requests v3 — role-gated onboarding */
(function(){'use strict';
if(!window.supabase)return;
const db=window.supabase.createClient('https://mzntgjyecymcpzciklfk.supabase.co','sb_publishable_AAXGC4EmiD4ELszpchz9Dw_Eryr6Usn',{auth:{persistSession:true,autoRefreshToken:true}});
const ROLES=['owner','gm','admin','editor'];
async function me(){const {data:{user}}=await db.auth.getUser();if(!user)return null;const {data,error}=await db.from('admin_users').select('role,is_active,display_name,username').eq('user_id',user.id).maybeSingle();if(error||!data?.is_active||!ROLES.includes(data.role))return null;return {...data,user};}
